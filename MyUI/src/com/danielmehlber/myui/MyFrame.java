...
public class MyFrame extends JFrame implements Designable {
	...
	public void paint(Graphics g) {
		...
		// g.drawImage(close, exit.getX() + topButtons.getX(),
		// exit.getY()+topButtons.getY(), exit.getWidth(), exit.getHeight(), null);
	}
	...
	public void animation_snap_in(boolean wait) {
		...
		Thread t = new Thread(() -> {
			while (getSize().height > 0) {
				MySyncTask.sync(120);
				setBounds(getX(), (int) (getY() + 20 * design.animation_speed / 2), getWidth(),
						(int) (getHeight() - 40 * design.animation_speed / 2));
			}
